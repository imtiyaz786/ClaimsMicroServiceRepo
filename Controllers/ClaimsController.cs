...
namespace ClaimsMicroservice.Controllers
{
    ...
    public class ClaimsController : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> GetClaimStatus([FromQuery] int claimID, [FromQuery] int policyID, [FromQuery] int memberID)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    return Ok(_claimRepository.GetClaimStatus(claimID, policyID, memberID));
                }
                else
        ...
        
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    return Ok(_claimRepository.SubmitClaim(policyID, memberID, benefitID, hospitalID, claimAmt, benefit));
                }
                else
                {
